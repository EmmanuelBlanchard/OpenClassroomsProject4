...
class CommentManager
{
    ...
    {
        $request = $this->database->prepare('SELECT comments.id, comments.pseudo, comments.comment, comments.comment_date, comments.post_id, posts.chapter FROM comments INNER JOIN posts ON posts.id = comments.post_id WHERE reported=1 ORDER BY posts.chapter, comments.comment_date DESC');
        $request->execute();
        $result = $request->fetchAll();
        return !$result ? null : $result;
    }
    ...
    {
        $request = $this->database->prepare('SELECT comments.id, comments.pseudo, comments.comment, comments.comment_date, comments.post_id, posts.chapter FROM comments INNER JOIN posts ON posts.id = comments.post_id WHERE approved=1 ORDER BY posts.chapter, comments.comment_date DESC');
        $request->execute();
        $result = $request->fetchAll();
        return !$result ? null : $result;
    }
    ...
    {
        $request = $this->database->prepare('SELECT comments.id, comments.pseudo, comments.comment, DATE_FORMAT(comments.comment_date, \'%e %M %Y à %H:%i\') AS comment_date_fr, comments.post_id, comments.reported, comments.approved, posts.chapter FROM comments INNER JOIN posts ON posts.id = comments.post_id WHERE comments.id = :id ORDER BY posts.chapter, comments.comment_date DESC');
