...
class PostManager
{
    ...
    {
        $request= $this->database->prepare('SELECT * FROM episodes WHERE id');
    ...
    {
        $request = $this->database->prepare('SELECT id, pseudo, comment, comment_created_the, episode_id FROM comments WHERE episode_id ORDER BY comment_created_the DESC');
